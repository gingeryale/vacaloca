...
router.post("/login", async (req, res, next) => {
  ...
  let userArr = await pool.query(
    `SELECT * FROM travel.users WHERE user_name='${name}' AND user_pass='${pass}'`
  );
  if (userArr.length > 0) {
...
router.get("/show", async function(req, res, next) {
  ...
  let result = await pool.query(`SELECT * FROM travel.vacations 
        ...
        subscribers.uid='1' ORDER BY subscribers.uid DESC`); //${req.session.connectedUser.id}
  res.json(result);
  //   } else {
  //     res.redirect("/login");
  //   }
...
router.get("/:id", async function(req, res, next) {
  ...
  let result = await pool.query(
    `SELECT * FROM travel.users WHERE id=${req.params.id}`
  );
  res.json(result);
  //   } else {
  //     res.redirect("/login");
...
router.post("/register", async function(req, res, next) {
  let insertQuery = ` INSERT INTO  travel.users (user_fname, user_lname, user_name, user_pass) 
    VALUES ('${req.body.fname}','${req.body.lname}','${req.body.name}','${
    req.body.pass
  }')`;
...
router.post("/subs/:id", async function(req, res, next) {
  ...
  let insertQuery = `INSERT into travel.subscribers(uid,vid) 
    VALUES ('1', '${req.params.id}')`; //${req.session.connectedUser.id}
...
router.delete("/subs/:vid", async function(req, res, next) {
  ...
  let result = await pool.query(`DELETE FROM travel.subscribers 
    ...
    uid='1'`); //${req.session.connectedUser.id}
  res.status(200).json({ msg: "OK" });
  //   } else {
  //     res.redirect("/");
  //   }
