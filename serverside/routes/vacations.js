...
router.get("/:id", async function(req, res, next) {
  let result = await pool.query(
    `SELECT * FROM travel.vacations WHERE id=${req.params.id}`
  );
  res.json(result);
});
...
router.delete("/:id", async function(req, res, next) {
  // if(req.session.connectedUser){
  let result = await pool.query(
    `DELETE FROM travel.vacations WHERE id=${req.params.id}`
  );
  res.status(200).json({ msg: "OK" });
  // } else {
  //     res.redirect('/');
...
router.post("/", async function(req, res, next) {
  ...
  let insertQuery = ` INSERT INTO  travel.vacations (vac_destination,vac_desc,vac_price,vac_checkin,vac_checkout, vac_img) 
    VALUES ('${req.body.destination}','${req.body.desc}',${req.body.price},'${
    req.body.checkin
  }',
